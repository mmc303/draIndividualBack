...
@router.get("/material/{nombre}")
def obtener_material_externo(nombre: str):
    url = f"https://genshin-db-api.vercel.app/api/v5/materials?query={nombre}&queryLanguages=spanish&resultLanguage=spanish"
    try:
        response = requests.get(url)
        response.raise_for_status()
        return response.json()
