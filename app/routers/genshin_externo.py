...
@router.get("/externo/weapons/{nombre}")
def obtener_arma_externa(nombre: str):
    url = f"https://genshin-db-api.vercel.app/api/v5/weapons?query={nombre}&queryLanguages=spanish&resultLanguage=spanish"
    try:
        response = requests.get(url)
        response.raise_for_status()
        return response.json()
...
@router.get("/externo/artifacts/{nombre}")
def obtener_artefacto_externo(nombre: str):
    url = f"https://genshin-db-api.vercel.app/api/v5/artifacts?query={nombre}&queryLanguages=spanish&resultLanguage=spanish"
    try:
        response = requests.get(url)
        response.raise_for_status()
        return response.json()
